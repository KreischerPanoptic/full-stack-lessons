...
export const DeleteUserModalComponent = () => {
    ...
    const [deleteVisible, setDeleteVisible] = useRecoilState(usersDeleteModalVisibleState)
    ...
    const handleDelete = async() => {
        ...
        axios.create().delete(`/api/users/remove/${request.id}`,{headers: {'Authorization' : `Bearer ${session?.accessToken}`}})
            .then(async (response) => {
                if (response.data.success) {
                    if(response.data.deleted) {
                        message.success(`Видалено користувача з логіном - ${username}`);
                        setDeleteVisible(false);
                        resetId();
                        resetUsername();
                        resetPassword();
    ...
    return (
        <Spin spinning={isLoading}>
            <Modal title={`Видалити користувача?`}
                   width='25%'
            ...
            >
                <p>{`Ви впевнені, що хочете видалити користувача з логіном - ${username}?`}</p>
            </Modal>
        </Spin>
    );
};
