...
export const SetLockoutUserModalComponent = () => {
    ...
    const [setVisible, setSetVisible] = useRecoilState(usersSetLockoutModalVisibleState);
    ...
    const handleSet = async() => {
        ...
        axios.create().get(`/api/users/set/lockout/${request.id}`,{headers: {'Authorization' : `Bearer ${session?.accessToken}`}})
            .then(async (response) => {
                if (response.data.success) {
                    if(response.data.set) {
                        message.success(`Блокування користувача з логіном - ${username} встановлено. Користувача заблоковано.`);
                        setSetVisible(false);
                        resetId();
                        resetUsername();
                        resetPassword();
    ...
    return (
        <Spin spinning={isLoading}>
            <Modal title={`Встановити блокування входу для користувача?`}
                   width='25%'
                   visible={setVisible}
                   okButtonProps={{
            ...
            >
                <p>{`Ви впевнені, що хочете встановити блокування входу для користувача з логіном - ${username}? Увага, користувач не зможе увійти використовуючи свій аккаунт, доки адміністратор сам не зніме блокування!`}</p>
            </Modal>
        </Spin>
    );
};
