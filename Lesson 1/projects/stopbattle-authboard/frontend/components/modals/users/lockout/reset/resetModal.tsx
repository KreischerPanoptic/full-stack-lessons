...
export const ResetLockoutUserModalComponent = () => {
    ...
    const [resetVisible, setResetVisible] = useRecoilState(usersResetLockoutModalVisibleState)
    ...
    const handleReset = async() => {
        ...
        axios.create().get(`/api/users/reset/lockout/${request.id}`,{headers: {'Authorization' : `Bearer ${session?.accessToken}`}})
            .then(async (response) => {
                if (response.data.success) {
                    if(response.data.reset) {
                        message.success(`Блокування користувача з логіном - ${username} знято. Користувач може зайти.`);
                        setResetVisible(false);
                        resetId();
                        resetUsername();
                        resetPassword();
    ...
    return (
        <Spin spinning={isLoading}>
            <Modal title={`Зняти блокування входу з користувача?`}
                   width='25%'
            ...
            >
                <p>{`Ви впевнені, що хочете зняти блокування входу з користувача з логіном - ${username}?`}</p>
            </Modal>
        </Spin>
    );
};
