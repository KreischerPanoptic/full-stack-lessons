...
export const EditUserModalComponent = () => {
    ...
    const [editVisible, setEditVisible] = useRecoilState(usersEditModalVisibleState)
    ...
    const handleUpdate = async() => {
        ...
        axios.create().post(`/api/users/update/${request.id}`, request,{headers: {'Authorization' : `Bearer ${session?.accessToken}`}})
            .then(async (response) => {
                if (response.data.success) {
    ...
    return (
        <Spin spinning={isLoading}>
            <Modal title="Редагувати користувача"
                   width='50%'
                   visible={editVisible}
                   okButtonProps={{
                       disabled: !(id > 0 && current && usernameValid && passwordValid
